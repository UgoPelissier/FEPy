...
def first_member_Dirichlet(triangulation, domain_Dirichlet, a):
    I_Points, D_Points = points_list_Dirichlet(triangulation, domain_Dirichlet)
    Points = I_Points+D_Points
...
def extract_submatrix_first_member(triangulation,domain_Dirichlet,a):
    A = first_member_Dirichlet(triangulation, domain_Dirichlet, a)
    I_Points, D_Points = points_list_Dirichlet(triangulation, domain_Dirichlet)
    I = len(I_Points)
    A_II = A[0:I,0:I]
    A_ID = A[0:I,I:len(A)]
...
def second_member_Dirichlet(triangulation, domain_Dirichlet, a, geometries):
    ...
    for i in range(len(F)):
        if i not in L:
            F_I.append(F[i])
    A_II, A_ID = extract_submatrix_first_member(triangulation,domain_Dirichlet,a)
...
def write_vtu(Triangulation, Points, U):
...
#--- PROBLEM ------------------------------------------------------------------+
A_II, A_ID = extract_submatrix_first_member(Delaunay_triangulation, bord_Dirichlet, a)
F_I = second_member_Dirichlet(Delaunay_triangulation, bord_Dirichlet, a, geom)

#--- SOLUTION -----------------------------------------------------------------+
U_I = np.linalg.solve(A_II, F_I)
U_D = dirichlet_vector(Delaunay_triangulation, bord_Dirichlet)
U = U_I.tolist() + U_D.tolist()
write_vtu(Delaunay_triangulation, Points, U)
