...
def elementary_diffusion_matrix(triangle, i, j):
    ...
    return round(coeff[0,0],10)

def elementary_convection_matrix(triangle, b, i, j):
    coeff = abs(aire(triangle))*np.matmul(b.transpose(),np.matmul(B(triangle),grad_phi_ref(j)))/6
    return round(coeff[0],10)
...
def convection_matrix(triangulation, b):
    Points = point_list(triangulation)
    ...
    for k in range(len(triangulation)):
        ...
        for i in range(3):
            ...
            for j in range(3):
                J = local2global(triangulation, Points, k, j)
                C[I,J] += elementary_convection_matrix(triangle, b, i, j)
    return C

def verif_mass_diffusion_matrix(triangulation):
    Points = point_list(triangulation)
    M = mass_matrix(triangulation)
    S = diffusion_matrix(triangulation)
    U = np.ones(len(Points)).transpose()
    r = np.matmul(np.matmul(U.transpose(),M),U)
    R = np.matmul(S,U)
    return r, R
...
#--- DIRICHLET MATRICES ASSEMBLY FUNCTIONS ------------------------------------+
def first_member_Dirichlet(triangulation, domain_Dirichlet, a, b):
    I_Points, D_Points = points_list_Dirichlet(triangulation, domain_Dirichlet)
    Points = I_Points+D_Points
    ...
    for k in range(len(triangulation)):
        ...
        for i in range(3):
            ...
            for j in range(3):
                J = local2global(triangulation, Points, k, j)
                A[I,J] += a*elementary_diffusion_matrix(triangle, i, j) + elementary_convection_matrix(triangle, b, i, j)
    return A
                
def extract_submatrix_first_member(triangulation, domain_Dirichlet, a, b):
    A = first_member_Dirichlet(triangulation, domain_Dirichlet, a, b)
    I_Points, D_Points = points_list_Dirichlet(triangulation, domain_Dirichlet)
    I = len(I_Points)
    A_II = A[0:I,0:I]
    A_ID = A[0:I,I:len(A)]
...
def second_member_Dirichlet(triangulation, domain_Dirichlet, a, b, geometries):
    ...
    for i in range(len(F)):
        if i not in L:
            F_I.append(F[i])
    A_II, A_ID = extract_submatrix_first_member(triangulation, domain_Dirichlet, a, b)
...
def plot_form_function(sommet, delta, points, triangulation, geometry):
    X = np.arange(-1, 1+delta, delta)
    Y = np.arange(-1, 1+delta, delta)
    X,Y = np.meshgrid(X,Y)
    Z = Phi_surf(sommet, points, X, Y, triangulation)
    plot_2d(X,Y,Z, triangulation, geometry)
...
def write_vtu(Triangulation, Points, U, n):
    path = '/Users/fp/Desktop/Ugo/Projets/Elements_Finis/Convection_Diffusion/2d/'
...
a = 1
theta = 80
b = 10*np.array([np.cos(np.deg2rad(theta)),np.sin(np.deg2rad(theta))])
...
#--- PROBLEM ------------------------------------------------------------------+
A_II, A_ID = extract_submatrix_first_member(Delaunay_triangulation, bord_Dirichlet, a, b)
F_I = second_member_Dirichlet(Delaunay_triangulation, bord_Dirichlet, a, b, geom)
