...
def correct_bad_triangles(delaunay_triangulation, geometries, min_angle, animation):
    already_good_triangulation = False
...
def multiple_geometries_delaunay_triangulation(geometries, min_angle, centers, animation, h):
    ...
    dt, state_bad_triangles = correct_bad_triangles(dt, geometries, min_angle, animation)
    plot_triangulaion(dt, geometries)
    plt.pause(0.1)
...
geom = [four, piece, c[0], c[1], c[2], c[3]]
